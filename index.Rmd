...
For outputting purpose, we may want to re-arrange variables' positions, and we can use `select(df, all_of(a_ch_vec))` or `select_at(df, .var = a_ch_vec)`. 
...
Not only does `group_by()` work with `summarise()`, but it also work with other `dplyr` functions. Take notice the following points:
